...
- The types implementation is also slightly awkward - where the Specifier is used to directly control the load/store instruction output, however this is only relevant for int, double and float types - can be called concrete types. However, there are many more specifiers / types that can be defined for use, eg chars, pointers, unsigned etc, which would use the same load / store instructions, but current enum implementation makes it messy - fix would be to have a layer of abstraction between the two.
